/**
 * ISO-3166-1 alpha-2 country codes + names + E.164 dial codes.
 *
 * The list is hand-curated rather than NPM-imported so the bundle
 * stays under tight control (the full pycountry-equivalent payload
 * is ~80 KB; this is ~6 KB). Order matters for "popular first" UX:
 * a few high-traffic markets sit at the top, the rest are
 * alphabetical.
 *
 * Used by the profile form (nationality / domicile) and the phone
 * country-code combobox.
 */

export interface Country {
  /** ISO-3166-1 alpha-2 (e.g. "IN", "US"). What the backend stores. */
  code: string;
  name: string;
  /** E.164 dial code without the leading "+". Multi-country dial codes (e.g. +1)
   *  are present on each entry; the user picks by country. */
  dial: string;
}
